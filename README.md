...
- Use `Region` to click up to 10 transit-area points. Click back near the start point after at least 3 points to close it, or let the 10th point close back to the start automatically. Select a region to edit its nodes and name; region labels display up to three lines.
...
- Edit `roadType`, `width`, `lanes`, `divider`, `Tunnel`, `oneWay`, direction, and `zLevel` in the right properties panel. Road types use OpenStreetMap-inspired classes: motorway, primary, secondary, tertiary, and residential. `Tunnel` is a separate checkbox that uses lighter road colors and dashed edges while keeping normal road controls.
...
- The map scale bar uses 50 m grid units and updates its displayed distance as you zoom.

## Road Type Defaults

| Road type | 1 lane | 2 lanes | 4 lanes | 6 lanes |
| --- | ---: | ---: | ---: | ---: |
| Motorway | 4 | 7 | 17 | 22 |
| Level 1 Primary road | 4 | 7 | 17 | 22 |
| Level 2 Secondary road | 4 | 7 | 17 | 22 |
| Level 3 Tertiary road | 4 | 7 | 17 | 22 |
| Residential road | 4 | 6 | 15 | 20 |
...
- Draw two residential roads on the same `zLevel` so they cross in an X or plus shape. The crossing should show a continuous road intersection.
- Draw a primary road and a residential road on the same `zLevel` so they form a T or plus shape. Road bodies should overlap naturally, while markings stop near the crossing.
...
- Enable roads with `lanes > 1` and `divider = true`. Lane markings and divider should not visibly run through the intersection center.
- Enable `Tunnel` on a road. It should render with a lighter road body and dashed edge stroke while preserving normal lane markings, divider, labels, selection, and node editing.

## Manual Connector Road Tests

- Case 1: Draw a `zLevel = 0` residential road, a `zLevel = 1` motorway/primary road, and one connector road. Set the connector `Road kind = Connector`, `Start zLevel = 0`, and `End zLevel = 1`. Place the start endpoint on the lower road and the end endpoint on the higher road. The connector start half should participate in `zLevel = 0` intersections, the end half should participate in `zLevel = 1` intersections, and the middle split point should be seamless without a black end cap or large overlay.
...
type RoadType = "motorway" | "primary" | "secondary" | "tertiary" | "residential";
